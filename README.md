...
* 小众Latex在线编译器推荐：[中科院中国科技云](https://latex.cstcloud.cn)。**不限**编译时长，**不限**checkpoint查看历史长度，**不限**同时协作人数，**免费**使用AI助手(默认模型是Qwen3.5-Next-80B，可以自己配置自己的API)
...
3. **设置电子签名（可选）**：如果您有签名图片，可以在导言区使用以下命令：
...
5. **管理图片**：将所有图片放入 `figures/` 文件夹，正文中直接引用文件名即可。
...
2. 任务书、学术诚信书、使用授权书、AI 工具使用说明页的标题默认采用 **方正小标宋简体二号** 字。模板会首先检测放在项目根目录的项目内字体文件（`FZXBSJW.ttf`），若不存在则使用黑体回退。
