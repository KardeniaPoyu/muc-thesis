...
2. 任务书、学术诚信书、使用授权书的标题默认采用 **方正小标宋简体二号** 字。模板会按以下顺序加载：系统字体“方正小标宋简体”→ 系统字体“方正小标宋简”→ 项目内字体文件（`方正小标宋简.TTF`、`方正小标宋简.ttf`，兼容 `方正小标宋简.TTF.ttf`；可放在项目根目录或 `fonts/` 目录）→ 黑体回退。若你已上传字体，建议按上述文件名放置。
